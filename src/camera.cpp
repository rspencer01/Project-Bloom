...
/// @param The window in which the mouse moves
/// Rotates the camera as per a FPS mouse control.
void Camera::getInputFromWindow(Window* win)
{
  glm::vec2 diff = win->getMouseCentreOffset();
  win->setMouseCentre();
  RotateX(-diff.y*0.005f);
  RotateFlat(-diff.x*0.005f);
}

/// @param The amount to zoom in
/// Moves the camera closer to the target object.
